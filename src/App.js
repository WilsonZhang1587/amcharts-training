...
export default function App() {
  ...
  function setObjectCoordinates(json) {
    ...
    const stores = json.result.map(data => ({
      latitude: Number(data.address.cnLatitude),
      longitude: Number(data.address.cnLongitude),
      title: data.description,
      fillColor: data.cn === '648' ? '#d10a28' : '#b114c9',
      // storeImage: data.cn === '648' ? testImage : 'null',
    }));
  ...
  //     const activeCircle2 = circle2Groups.find(data => data.dataItem.dataContext.cn === message.cn);
  //     // 單次閃爍
  ...
  //         if (activeCircle2.dataItem.dataContext.cn === this.props.activeBranch.cn) {
